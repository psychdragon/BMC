...
$consolidatedData = '';
$requiredFiles = ['BMC' => $bmcFilePath, 'SWOT' => $swotFilePath, 'Proposal' => $proposalFilePath];
$optionalFiles = ['Roadmap' => $roadmapFilePath];

// Handle required files
foreach ($requiredFiles as $name => $path) {
    if (!file_exists($path)) {
        http_response_code(404);
        echo json_encode(['status' => 'error', 'message' => "Required source document ($name) not found."]);
        exit;
    }
...
foreach ($optionalFiles as $name => $path) {
    if (file_exists($path)) {
        $consolidatedData .= "$name:\n" . file_get_contents($path) . "\n\n";
    }
}
...
$systemPrompt = file_get_contents('system_prompt_plan.md');
$prompt = $systemPrompt . "\n\nHere is the consolidated project data:\n\n" . $consolidatedData;
...
if (file_put_contents($planFilePath, json_encode($decodedContent, JSON_PRETTY_PRINT))) {
    ...
    http_response_code(500);
    echo json_encode(['status' => 'error', 'message' => 'Failed to save the generated project plan file.']);
}
